...
class Queries extends CI_Model {
    ...
    public function get_jobs($page,$order_by,$city_id,$category_id,$edu_id,$salary,$work_time,
                $worker_type_id,$is_student,$is_school,$is_pensioneer,$is_disabled,$is_shift,$no_exp)
    {
        $this->db->select('jobs.id AS jid, markers.id AS mid, markers.lat, markers.lng,
        companies.average_salary AS avg_sal, jobs.salary_from, jobs.salary_to,
         companies.high_credit_rating AS credit');
        ...
        $this->db->select('jobs.id AS jid,company_id,title,companies.name AS company,companies.logo AS logo');
        $this->db->from('jobs');
        $this->db->join('companies','companies.id = jobs.company_id');
        ...
        for($i=0; $i<sizeof($data); $i++){
            $data[$i]["jid"] = intval($data[$i]["jid"]);
            $data[$i]["mid"] = intval($data[$i]["mid"]);
            for($j=0; $j<sizeof($data2); $j++){
                if($data[$i]["jid"] == $data2[$j]["jid"]){
                    $data[$i]["title"] = $data2[$j]["title"];
                   $data[$i]["company_id"] = intval($data2[$j]["company_id"]);
                   $data[$i]["company"] = $data2[$j]["company"];
                   $data[$i]["logo"] = $data2[$j]["logo"];
                }
            }
        }
    ...
    public function get_jobs2($page,$query_text,$order_by,$city_id,$category_id,$edu_id,$salary,$new,$premium,$work_time,
            ...
            $worker_type_id,$is_student,$is_school,$is_pensioneer,$is_disabled,$is_shift,$no_exp)
    {
        ...
        if(!empty($salary)){
            $this->db->where("salary_to >=", $salary);
        }
