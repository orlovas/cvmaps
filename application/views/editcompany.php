...
echo form_open_multipart('?c=backoffice&m=edit_company&id='.$id);
...
<input type="text" name="average_salary" id="average_salary" value="<?php echo $average_salary ?>" required><br />
...
<select name="high_credit_rating" id="high_credit_rating" required>
    <option value="1" <?php echo ($high_credit_rating === "1" ? "selected" : "") ?>>Taip</option>
    <option value="0" <?php echo ($high_credit_rating === "0" ? "selected" : "") ?>>Ne</option>
</select>
<br />
<label for="logo">Logotipas</label> <span id="enable_upload">Užkrauti naują logotipą</span>
<div id="upload"></div>
<input type="hidden" name="logo" id="logo" value="<?php echo $logo ?>">
<?php echo '<img src="static/images/l/'.$logo.'">'; ?>
